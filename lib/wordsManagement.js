...
function groupWordsByLetterPositionAndLength(words, position) {
    const groups = {};
...
function selectGroups(groups, count, maxLen) {
    ...
    while (selectedGroups.length < count && keys.length > 0) {
        ...
        const obj = {
            used: [],
            toUse: group.slice(0, maxLen),
            getWords: (maxLength) => {
                if (obj.used.length >= maxLength) {
                    return [];
                }
                return obj.toUse;
            }
...
function libraryGroupsContainsWord(word) {
...
function createSession(width, height, maxLongWordCount, wordCountWithSameLetter, groupCountWidthSameLetter) {
    ...
    const sameLetterIndexes = randomNumbers(groupCountWidthSameLetter, 0, groupCountWidthSameLetter);
    let finalGroups = [];

    function getGroups(pattern) {
        const arr = [];
        for (let i = 0; i < pattern.length; i++) {
            const groups = groupWordsByLetterPositionAndLength(filteredLibrary, pattern[i]);
