...
function pickRandomWords(words, count) {
    ...
    while (selectedWords.length < count && words.length > 0) {
        ...
        words.splice(randomIndex, 1); // Удаляем выбранное слово, чтобы не выбрать его снова
    }
...
function createSession(width, height, maxLongWordCount, wordCountWithSameLetter, groupCountWidthSameLetter) {
    ...
    // const { shortWords, mediumWords, longWords } = categorizeWords(filteredLibrary);

    // const maxCount = (parseInt(width) + parseInt(height)) * 2;

    // const numLongWords = Math.floor(maxCount * 0.30);
    // const numMediumWords = Math.floor(maxCount * 0.0);
    // const numShortWords = maxCount - numLongWords - numMediumWords;

    // const selectedLongWords = pickRandomWords(longWords, numLongWords);
    // const selectedMediumWords = pickRandomWords(mediumWords, numMediumWords);
    // const selectedShortWords = pickRandomWords(shortWords, numShortWords);

    // console.log(`${numLongWords} (25%) - ${selectedLongWords.length}`);
    // console.log(`${numMediumWords} (40%) - ${selectedMediumWords.length}`);
    // console.log(`${numShortWords} (35%) - ${selectedShortWords.length}`);


    // usingLibrary = {
    //     filtered: [...selectedLongWords, ...selectedMediumWords, ...selectedShortWords],
    //     groups: [],
    //     remove: (word) => {
    //         usingLibrary.filtered = removeSameWords(usingLibrary.filtered, [word]);
    //     }
    // };
    // console.log(usingLibrary.filtered.length);
    ...
    const maxWordLength = Math.max(6, 3)
