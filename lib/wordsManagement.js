...
function groupWordsByLetterPositionAndLength(words, position, maxGridSide) {
    const groups = {};

    const filtered = filterWordsByLength(words, 3, Math.max(maxGridSide - 2, 3))
...
function createSession(width, height, maxLongWordCount, wordCountWithSameLetter, groupCountWidthSameLetter) {
    ...
    function getGroups(pattern) {
        ...
        for (let i = 0; i < pattern.length; i++) {
            const groups = groupWordsByLetterPositionAndLength(filteredLibrary, pattern[i], Math.max(width, height));
