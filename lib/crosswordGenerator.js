...
function getWord(density) {
    if (usingLibrary.categorized == null) {
        const word = usingLibrary.filtered[randomInt(0, usingLibrary.filtered.length - 1)]
        return [{ word: word, type: 'default' }];
    }
...
function moveByGridAndTryPlaceWords(grid, words) {
    for (let i = 0; i < words.length; i++) {
        ...
        for (let d = 0; d < directions.length; d++) {
            ...
            if (direction == Direction.horizontal) {
                for (let x = 0; x < grid.size.x - word.length + 1; x++) {
                    for (let y = 0; y < grid.size.y; y++) {
                        const createdWord = createWordInPosition(grid, word, x, y, [direction]);
                        if (createdWord != null) {
                            return createdWord;
                        }
                    }
                ...
                for (let x = 0; x < grid.size.x; x++) {
                    for (let y = 0; y < grid.size.y - word.length + 1; y++) {
                        const createdWord = createWordInPosition(grid, word, x, y, [direction]);
                        if (createdWord != null) {
                            return createdWord;
                        }
...
function tryInsertWord(grid, density, iteration) {
    ...
    let placedWord = moveByGridAndTryPlaceWords(grid, words);
...
async function generateCrossword() {
    ...
    while (!completedConditions) {
        for (let it = 0; it < gridGenerationIteration; it++) {
            ...
            if (grid.density >= 0 && grid.groups.sum == sameLetterGroupCount * sameLetterWordCount) {
