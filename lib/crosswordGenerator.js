const maxGridCount = 10;
const normalGridDensity = 0.65;
const generationInterations = 20;
const wordSelectionIterations = 200;
let gridWidth = 5;
let gridHeigth = 5;

const longWordThreshold = 12;
const maxLongWords = 10;
...
function hasNeighborAtPosition(grid, x, y, direction) {
    if (direction == Direction.vertical) {
        for (let xs = -1; xs <= 1; xs++) {
            if (xs != 0 && getCellCharacter(grid, x + xs, y) != null) {
                return true;
            }
        }
    } else {
        for (let ys = -1; ys <= 1; ys++) {
            if (ys != 0 && getCellCharacter(grid, x, y + ys) != null) return true;
        }
    }
    return false;
}
...
function tryInsertWord(grid, density, iteration) {
    ...
    if (grid.words.length == 0) {
        return createWord(word, 0, 0, randomInt(0, 2) == 0 ? Direction.horizontal : Direction.vertical);
    }
    let horizontalFirstly = true;
    for (let i = 0; i < grid.words.length; i++) {
        const gridWord = grid.words[i];
        for (let j = 0; j < gridWord.word.length; j++) {
            const x = gridWord.position.x + (gridWord.direction == Direction.horizontal ? j : 0);
            const y = gridWord.position.y + (gridWord.direction == Direction.vertical ? j : 0);

            for (let k = 0; k < word.length; k++) {
                if (gridWord.direction == Direction.horizontal) {
                    if (!tryPlaceWord(grid, word, x, y - k, Direction.vertical)) {
                        continue;
                    }
                    return createWord(word, x, y - k, Direction.vertical);
                } else {
                    if (!tryPlaceWord(grid, word, x - k, y, Direction.horizontal)) {
                        continue;
                    }
                    return createWord(word, x - k, y, Direction.horizontal);
                }
            }
        }
    }
...
async function generateCrossword() {
    ...
    function getGridWithHighestDinsity() {
        if (gridList.length == maxGridCount) {
            ...
            for (let i = 1; i < gridList.length; i++) {
                ...
                if (element.density > highest.density) {
                    highest = element;
                }
            }
            return highest;
    ...
    document.getElementById("density-title").innerText = `Density: ${Math.floor(finalGrid.density * 100)}%`;
...
function generate() {
    ...
    for (let it = 0; it < 1; it++) {
        for (let iteration = 0; iteration < generationInterations; iteration++) {
            ...
            grid.words = grid.words.sort(() => Math.random() - 0.5);
        }
    }
