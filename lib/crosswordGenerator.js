...
const wordSelectionIterations = 2;
const wordCountToCheck = 1;
let gridWidth = 5;
let gridHeigth = 5;
...
function tryInsertWord(grid, density, iteration) {
    ...
    function getWordsForPlacement() {
        const words = [];
        for (let i = 0; i < wordCountToCheck; i++) {
            words.push(getWord(density));
        }
        return words;
    }
...
async function generateCrossword() {
    ...
    for (let x = 0; x < finalGrid.size.x; x++) {
        for (let y = 0; y < finalGrid.size.y; y++) {
            ...
            const cellElement = gridElement.children[x + y * finalGrid.size.x];
    ...
    for (let i = 0; i < finalGrid.words.length; i++) {
        const word = finalGrid.words[i];
        const cellElement = gridElement.children[word.position.x + word.position.y * finalGrid.size.x];
        if (cellElement)
...
function createField(x, y) {
    ...
    console.log(`${gridWidth}  ${gridHeigth}  ${ratio}`);
