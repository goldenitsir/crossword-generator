...
function getWord(density) {
    ...
    if (!startWithGroupedWords && density > .01 || startWithGroupedWords && (noGroupedWords != null && noGroupedWords)) {
...
function getCellPriority(grid, x, y) {
    ...
    for (let i = 0; i < grid.words.length; i++) {
        ...
        for (let j = 0; j < gridWord.word.length; j++) {
            ...
            if (tx == x && ty == y && libraryGroupsContainsWord(gridWord.word)) {
                return 1;
            };
        }
    }
...
function createWordInPosition(grid, word, x, y, direction) {

    if (!tryPlaceWord(grid, word, x, y, direction == Direction.horizontal ? Direction.horizontal : Direction.vertical)) {
        if (!tryPlaceWord(grid, word, x, y, direction == Direction.horizontal ? Direction.vertical : Direction.horizontal)) {
            return null;
        }
        return createWord(word, x, y, direction == Direction.horizontal ? Direction.vertical : Direction.horizontal);
    }
    return createWord(word, x, y, direction == Direction.horizontal ? Direction.horizontal : Direction.vertical);
}

function moveByGridAndTryPlaceWord(grid, words, start, step) {
    for (let i = 0; i < words.length; i++) {
        ...
        for (let x = start; x < grid.size.x - start; x += step) {
            for (let y = start; y < grid.size.y - start; y += step) {
                ...
                const createdWord = createWordInPosition(grid, word, x, y, horizontalFirstly ? Direction.horizontal : Direction.vertical);
...
function tryInsertWord(grid, density, iteration) {
    ...
    let placedWord = moveByGridAndTryPlaceWord(grid, words, 0, 1);
